...
def draw_3graph(title, subtitle, position, (actualValue, minValue, maxValue)):  # Designed to draw 3 graph on top of each other
	...
	draw.rectangle((0, graph3Y[position], titleSize[0] + 6, graph3Y[position] + 18), outline=255,
	...
	draw.rectangle((titleSize[0] + 6, graph3Y[position] + 8, 127, graph3Y[position] + 18), outline=255,
				   fill=0)  # Empty graph

	fill = scale_value(actualValue, minValue, maxValue, 125 - (titleSize[0] + 8))
	draw.rectangle((titleSize[0] + 8, graph3Y[position] + 10, (titleSize[0] + 8) + fill, graph3Y[position] + 16),
...
def render_bars(serverip):
	...
	while True:
		...
		with globalVars.canvas(globalVars.device) as draw:
			draw_3graph("TMP", "CPU", 0, (temp, 25, 60))
			draw_3graph("LOD", "CPU", 1, (load, 0, 100))
			draw_3graph("CLK", "CPU", 2, (clock, 0, 3090))
			sleep(1)
